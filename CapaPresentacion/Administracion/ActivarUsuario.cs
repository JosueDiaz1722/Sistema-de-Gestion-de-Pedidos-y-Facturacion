...
namespace CapaPresentacion.Administracion
{
    public partial class ActivarUsuario : Form
    {
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                if (UsuariodataGridView.CurrentRow.Cells[3].Value.ToString().Equals("ACTIVO"))
                {
                    MessageBox.Show("Usuario Ya Esta Activo");
                }
                else
                {
                    String Men = "¿Seguro que quiere activar a este usuario?";
                    String Tit = "Activar Usuario";
                    MessageBoxButtons buttons = MessageBoxButtons.YesNo;
                    DialogResult result = MessageBox.Show(Men, Tit, buttons);
                    if (result == DialogResult.Yes)
                    {
